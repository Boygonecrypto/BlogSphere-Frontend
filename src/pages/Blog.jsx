...
const Blog = () => {
  ...
  const fetchComments = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log(data.message);
        toast.error("blog Id is not found");
      } else {
        toast.error(data.message);
      }
    } catch (error) {
  ...
  const addComment = async (e) => {
    e.preventDefault();
    try {
      const response = await axiosInstance.post("/blog/add-comment", {
        blog: id,
        name,
        content,
      });
      ...
      <div className="mx-5 max-w-5xl md:mx-auto my-10 mt-6">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            <button
              type="submit"
              className="bg-primary p-2 text-white rounded px-8 hover:scale-102 transition-all cursor-pointer"
            >
              Submit
            </button>
          </form>
